...
'email_logs': {},
    ...
    for meet_info in meet_infos:
        ...
        tzname = meet.infos['default_time_zone'] or results['organization'].infos['default_time_zone'] or settings.CLIENT_DEFAULT_TIME_ZONE
        time_zone = pytz.timezone(tzname)
        results['tzname'] = tzname
        results['time_triggered'] = begin.astimezone(time_zone).strftime('%Y-%m-%d %H:%M%p')
        mold = MessageTemplate.objects.filter(type='batch_create_gatherings', organization=results['organization']).first()

        if not mold:
            results['explain'] = f"MessageTemplate with type 'batch_create_gatherings' under Organization '{results['organization']}' cannot be found."
            if meet_info['recipients'] and type(meet_info['recipients']) is list:
                for recipient in meet_info['recipients']:
                    results['recipient'] = recipient
                    results['email_status'][recipient] = mail_result(results)
        ...
        if not meet_info['months_adding']:
            results['explain'] = f"'months_adding' in the PeriodicTask argument JSON cannot be found or invalid."
            if meet_info['recipients'] and type(meet_info['recipients']) is list:
                for recipient in meet_info['recipients']:
                    results['recipient'] = recipient
                    results['email_status'][recipient] = mail_result(results)
