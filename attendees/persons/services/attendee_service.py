...
class AttendeeService:
    ...
    @staticmethod
    def by_datagrid_params(current_user, assemblies, orderby_string, filters_list):
        """
        ...
        :param current_user:
        :param assemblies: attendee participated assembly ids. Exception: if assembly is in organization's all_access_assemblies, all attendee of the same org will be return
        :param orderby_string:
        :param filters_list:
        :return:
        """
        orderby_list = AttendeeService.orderby_parser(orderby_string, assemblies)

        init_query = Q(division__organization=current_user.organization)
        # Todo: need filter on attending_meet finish_date

        final_query = init_query.add(AttendeeService.filter_parser(filters_list, assemblies), Q.AND)
    ...
    @staticmethod
    def orderby_parser(orderby_string, assemblies):
        """
        ...
        :param assemblies: assembly ids
        :return: a List of sorter for order_by()
        """
        meet_sorters = {meet.slug: Func(F('attendingmeets'), function="'{}'=ANY".format(meet.slug)) for meet in Meet.objects.filter(assembly__in=assemblies)}
    ...
    @staticmethod
    def filter_parser(filters_list, assemblies):
        """
        ...
        if filters_list:
            if and_string in filters_list and or_string in filters_list:
                ...
                and_query = AttendeeService.filter_parser(and_list[0], assemblies)
                for and_element in and_list[1:]:
                    and_query.add(AttendeeService.filter_parser(and_element, assemblies), Q.AND)
                return and_query
            elif filters_list[1] == or_string:
                or_list = [element for element in filters_list if element != or_string]
                or_query = AttendeeService.filter_parser(or_list[0], assemblies)
                for or_element in or_list[1:]:
                    or_query.add(AttendeeService.filter_parser(or_element, assemblies), Q.OR)
                return or_query
            elif filters_list[1] == '=':
                return Q(**{filters_list[0].replace('.', '__'): filters_list[2]})
            elif filters_list[1] == 'contains':
                return Q(**{AttendeeService.field_convert(filters_list[0], assemblies) + '__icontains': filters_list[2]})
        return Q()

    @staticmethod
    def field_convert(query_field, assemblies):
        """
        ...
        if assemblies:
            for meet in Meet.objects.filter(assembly__in=assemblies):
